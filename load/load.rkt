...
(define (list->transaction sexp line-number)
  (if (list? sexp)
      (make-transaction (first sexp)
                        ...
                        (if (> (length sexp) 5)
                            (sixth sexp)
                            empty))
      (error (format "line ~a: not a list: ~a" line-number sexp))))
      ...
      (lambda (p)
        (let loop ((sexp (read p))
          ...
          (if (eof-object? sexp)
              (reverse result)
              (loop (read p)
                    (begin (set! line-number (+ 1 line-number))
                           (cons (list->transaction sexp line-number) result)))))))))
